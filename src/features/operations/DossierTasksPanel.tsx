...
function TaskFormDialog({
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return editing
        ? api.patch<WorkTask>(
            `/api/organizations/${organizationId}/dossiers/${dossierId}/tasks/${task?.id}`,
            payload,
          )
        : api.post<WorkTask>(
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: ["dossier-tasks", organizationId, dossierId],
      });
      onClose();
    },
    onError: (reason) =>
    ...
    >
      <DialogTitle>
        <Typography variant="h3">
          {editing ? "Modifier la tâche" : "Créer une tâche"}
        </Typography>
      </DialogTitle>
        ...
        <Box sx={{ display: "grid", gap: 2 }}>
          ...
          >
            <TextField
              ...
              value={values.dueOn}
              onChange={(event) =>
                setValues({ ...values, dueOn: event.target.value })
              }
          ...
          {!editing && (
            <TextField
              ...
              onChange={(event) =>
                setValues({ ...values, checklist: event.target.value })
              }
...
function TaskDetailDialog({
  ...
  const comments = useQuery({
    ...
    queryFn: () =>
      api.get<TaskComment[]>(
        `/api/organizations/${organizationId}/dossiers/${dossierId}/tasks/${task?.id}/comments`,
      ),
  ...
  const action = useMutation({
    mutationFn: async ({
      ...
      if (type === "start")
        return api.patch(base + "/progress", { status: "EN_COURS" });
      if (type === "submit")
        return api.patch(base + "/progress", {
          status: "PRETE_POUR_REVISION",
          comment: comment.trim() || null,
        });
      if (type === "complete") return api.post(base + "/complete");
      if (type === "reject")
        return api.post(base + "/reject", { comment: rejectComment.trim() });
      if (type.startsWith("assign:"))
        return api.put(base + `/assignee/${type.slice(7)}`);
      if (type.startsWith("check:"))
        return api.patch(base + `/checklist/${type.slice(6)}`, payload);
      if (type === "add-check")
        return api.post(base + "/checklist", { label: newItem.trim() });
      if (type === "comment")
        return api.post(base + "/comments", { body: comment.trim() });
    },
    onSuccess: async (_, variables) => {
      setError("");
      if (variables.type === "add-check") setNewItem("");
      if (variables.type === "comment" || variables.type === "submit")
        setComment("");
            ...
            <Stack spacing={2.5}>
              ...
              <Box>
                  ...
                  <FormControlLabel
                    ...
                    control={
                      <Checkbox
                        ...
                        disabled={!canManage || closed || action.isPending}
                        onChange={(_, checked) =>
                          action.mutate({
                            type: `check:${item.id}`,
                ...
                {task.checklist.length === 0 && (
                  <Typography variant="body2" color="text.secondary">
                    Aucun contrôle ajouté pour cette tâche.
                  </Typography>
                )}
                {canManage && !closed && (
                  <Box sx={{ display: "flex", gap: 1, mt: 1 }}>
                    <TextField
                      size="small"
                      fullWidth
                      placeholder="Nouvel élément de contrôle…"
                      value={newItem}
                      onChange={(event) => setNewItem(event.target.value)}
                    />
                    <Button
                      onClick={() => action.mutate({ type: "add-check" })}
                      disabled={!newItem.trim() || action.isPending}
                    >
                      Ajouter
                    </Button>
                  </Box>
              ...
              <Box>
                ...
                {comments.data?.map((entry) => (
                  <Box
                    key={entry.id}
                    sx={{ p: 1.5, mb: 1, bgcolor: "#f7f5ef", borderRadius: 2 }}
                  >
                    <Typography variant="caption" sx={{ fontWeight: 700 }}>
                      {entry.authorName ?? "Utilisateur"} ·{" "}
                ...
                {canManage && (
                  <Box sx={{ display: "flex", gap: 1, mt: 1 }}>
                    ...
                    <IconButton
                      color="primary"
                      onClick={() => action.mutate({ type: "comment" })}
            ...
            <Stack spacing={2}>
                ...
                <Box sx={{ p: 2 }}>
                  ...
                  {canAssign && !closed && (
                    <TextField
                      ...
                      onChange={(event) =>
                        action.mutate({ type: `assign:${event.target.value}` })
                      }
                      sx={{ mt: 2 }}
                    >
                      {assignments
              ...
              <Card variant="outlined">
                <Box sx={{ p: 2 }}>
                  ...
                  <Stack spacing={1}>
                    {canManage && task.status === "A_FAIRE" && (
                      <Button
                        variant="contained"
                        startIcon={<PlayArrowRounded />}
                        onClick={() => action.mutate({ type: "start" })}
                      >
                        Démarrer
                      </Button>
                    )}
                    {canManage && task.status === "EN_COURS" && (
                      <Button
                        variant="contained"
                    ...
                    {canValidate && task.status === "PRETE_POUR_REVISION" && (
                      <Button
                        variant="contained"
                        color="success"
              ...
              {task.lastComment && (
                <Alert severity="info">{task.lastComment}</Alert>
              )}
            </Stack>
          </Box>
        </DialogContent>
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      api.get<PagedResponse<WorkTask>>(
        `/api/organizations/${organizationId}/dossiers/${dossierId}/tasks?page=1&pageSize=100${status ? `&status=${status}` : ""}`,
      ),
  });
  ...
  const stats = useMemo(
    () => ({
      overdue: tasks.filter(
        (task) =>
          task.isOverdue && !["TERMINEE", "ANNULEE"].includes(task.status),
      ).length,
      review: tasks.filter((task) => task.status === "PRETE_POUR_REVISION")
        .length,
    ...
    <>
      <Card>
        <Box sx={{ p: { xs: 2, md: 2.5 } }}>
          ...
          >
            <Box>
              <Typography variant="h3">Production du dossier</Typography>
              <Typography variant="body2" color="text.secondary">
          ...
          >
            ...
            <TaskKpi
              label="À réviser"
              value={stats.review}
              tone={stats.review ? "warning" : "success"}
            />
        ...
        {!query.isLoading && tasks.length === 0 && (
          ...
          >
            ...
            <Typography
              variant="body2"
              color="text.secondary"
              sx={{ maxWidth: 520, mx: "auto", mt: 0.5 }}
            >
              Créez une première tâche manuelle ou générez les obligations
              fiscales pour cadrer le travail du mois.
            </Typography>
            {canManage && !archived && (
              <Button
                variant="contained"
            ...
            >
              ...
              <Box sx={{ minWidth: 0 }}>
                ...
                <Stack
                  direction="row"
                  spacing={1.25}
                  sx={{ mt: 0.45, color: "text.secondary", flexWrap: "wrap" }}
                >
