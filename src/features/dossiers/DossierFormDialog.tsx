...
const money = z
  .string()
  .refine(
    (value) => !value || /^\d{1,12}(\.\d{1,3})?$/.test(value),
    "Montant invalide (3 décimales maximum).",
  );
const validTunisianTaxIdentifier = (value: string) =>
  !value ||
  /^\d{7,8}\s*\/?\s*[A-Z]\s*\/?\s*[A-Z]\s*\/?\s*\d{3}$/i.test(value.trim());
...
const dossierSchema = z.object({
  ...
  taxIdentifier: z
    .string()
    .max(100)
    .refine(validTunisianTaxIdentifier, "Format MF attendu : 1234567/A/M/000."),
  rneNumber: z
    .string()
    .max(100)
    .refine(validRneNumber, "RNE invalide : lettres/chiffres uniquement."),
...
export function DossierFormDialog({
  ...
  const mutation = useMutation({
    mutationFn: (values: DossierFormValues) => {
      ...
      return editing
        ? api.patch<DossierSummary>(
            `/api/organizations/${organizationId}/dossiers/${dossier?.id}`,
            payload,
          )
    ...
    <>
      ...
      <DialogTitle sx={{ pb: 1 }}>
        <Typography
          ...
          sx={{ fontWeight: 700, letterSpacing: ".12em" }}
        >
          {editing ? "Mise à jour" : "Nouveau client"}
        </Typography>
        <Typography variant="h3">
          {editing ? "Modifier le dossier" : "Créer un dossier client"}
        </Typography>
      </DialogTitle>
      <DialogContent dividers>
        ...
        >
          <Box>
            ...
            >
              ...
              <TextField
                ...
                helperText={
                  errors.taxIdentifier?.message ?? "Exemple : 1234567/A/M/000"
                }
                {...register("taxIdentifier")}
              />
              <TextField
                label="Numéro RNE"
                error={Boolean(errors.rneNumber)}
                helperText={errors.rneNumber?.message}
                {...register("rneNumber")}
              />
              <Controller
                name="legalForm"
                control={control}
                render={({ field }) => (
                  <TextField select label="Forme juridique" {...field}>
              ...
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <TextField select label="Régime fiscal" {...field}>
          ...
          <Box>
            ...
            <Box
              sx={{ display: "flex", flexWrap: "wrap", gap: { xs: 1, sm: 3 } }}
            >
              <Controller
                name="isVatSubject"
                control={control}
          ...
          <Box>
            ...
            >
              ...
              <TextField
                ...
                error={Boolean(errors.fiscalYearStartMonth)}
              ...
              <TextField
                label="Étiquettes"
                placeholder="TVA, paie, export…"
                helperText="Séparez les étiquettes par une virgule."
                sx={{ gridColumn: { sm: editing ? "span 2" : "span 3" } }}
                {...register("tags")}
              />
              <TextField
