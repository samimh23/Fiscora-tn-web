...
export const legalFormLabel = (value: string) =>
  legalFormOptions.find((item) => item.value === value)?.label ?? value;
export const taxRegimeLabel = (value: string) =>
  taxRegimeOptions.find((item) => item.value === value)?.label ?? value;
export const billingFrequencyLabel = (value?: string) =>
  billingFrequencyOptions.find((item) => item.value === value)?.label ??
  value ??
  "—";
