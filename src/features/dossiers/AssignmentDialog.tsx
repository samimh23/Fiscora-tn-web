      ...
      <DialogContent dividers>
        ...
        >
            ...
            render={({ field }) => (
              <TextField
                ...
                onChange={(event) => {
                  ...
                  if (member?.role === "Portail client")
                    setValue("assignmentRole", "CLIENT");
                  else if (assignmentRole === "CLIENT")
                    setValue("assignmentRole", "SUPPORT");
                }}
              >
                {members
                  .filter(
                    (member) =>
                      member.isActive ||
                      member.membershipId === assignment?.membershipId,
                  )
                  .map((member) => (
                    <MenuItem
          ...
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <TextField select label="Rôle sur le dossier" {...field}>
                <MenuItem value="RESPONSABLE">Responsable</MenuItem>
                <MenuItem value="SUPPORT">Support</MenuItem>
                <MenuItem value="CLIENT">Accès portail client</MenuItem>
              </TextField>
            )}
          />
          {assignmentRole !== "CLIENT" && (
            <TextField
              type="number"
              label="Budget mensuel (heures)"
              slotProps={{ htmlInput: { min: 0, step: 0.25 } }}
              error={Boolean(errors.monthlyTimeBudgetHours)}
              helperText={
                errors.monthlyTimeBudgetHours?.message ??
                "Utilisé pour suivre la charge et la rentabilité du dossier."
              }
              {...register("monthlyTimeBudgetHours", { valueAsNumber: true })}
            />
          )}
          {assignmentRole === "CLIENT" && (
            <Alert severity="info">
              Ce client pourra voir ce dossier, ses pièces et ses échéances dans
              son espace client.
            </Alert>
          )}
          {editing && (
