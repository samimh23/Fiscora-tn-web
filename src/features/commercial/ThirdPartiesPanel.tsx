...
export function ThirdPartiesPanel({
  organizationId,
  dossierId,
  parties,
  accounts,
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return selected
        ? api.put<ThirdParty>(path, payload)
        : api.post<ThirdParty>(path, payload);
    },
    onSuccess: async () => {
    ...
    onError: (reason) =>
      setError(
        reason instanceof ApiError
          ? reason.message
          : t("Impossible d’enregistrer ce tiers."),
      ),
  });
  ...
  return (
    <>
      <Card>
        ...
        >
          <Box>
            <Typography variant="h3">{t("Clients et fournisseurs")}</Typography>
            <Typography variant="body2" color="text.secondary">
              {t("Fiches tiers, coordonnées, adresses et soldes comptables.")}
            </Typography>
          </Box>
          {canManage && !archived && (
            <Button
        ...
        {!loading && parties.length === 0 && (
          <Box sx={{ p: 6, textAlign: "center" }}>
            <BusinessOutlined sx={{ fontSize: 46, color: "text.disabled" }} />
            <Typography sx={{ fontWeight: 700, mt: 1 }}>
              {t("Aucun tiers enregistré")}
            </Typography>
            <Typography variant="body2" color="text.secondary">
              {t(
        ...
        {parties.map((party) => (
          <Box
            ...
            sx={{
              ...
              gridTemplateColumns: {
                xs: "1fr",
                md: "minmax(260px, 1fr) 170px 170px auto",
              },
              gap: 2,
              alignItems: "center",
            }}
          >
            ...
            >
              ...
              <Box sx={{ minWidth: 0 }}>
                ...
                >
                  <Chip
                    label={t(partyTypeLabels[party.type])}
                    size="small"
                    variant="outlined"
                  />
                  {party.taxIdentifier && (
                    <Typography variant="caption" color="text.secondary">
                      MF {party.taxIdentifier}
                    </Typography>
                  )}
                </Stack>
                <Typography variant="caption" color="text.secondary">
            ...
            <Box>
              ...
              <Typography
                sx={{
                  fontWeight: 700,
                  color:
                    Number(party.receivableBalance) > 0
                      ? "success.dark"
                      : "text.primary",
                }}
              >
                {money(party.receivableBalance)}
              </Typography>
            </Box>
            <Box>
              <Typography variant="caption" color="text.secondary">
              ...
              <Typography
                sx={{
                  fontWeight: 700,
                  color:
                    Number(party.payableBalance) > 0
                      ? "warning.dark"
                      : "text.primary",
                }}
              >
                {money(party.payableBalance)}
              </Typography>
            </Box>
            {canManage && !archived && (
              <Button
      ...
      >
        <DialogTitle>
          {selected
            ? t("Modifier le tiers")
            : t("Ajouter un client ou fournisseur")}
        </DialogTitle>
        <DialogContent
          sx={{
            display: "grid",
        ...
        >
          ...
          <TextField
            ...
            value={form.type}
            onChange={(event) =>
              set("type", event.target.value as Form["type"])
            }
          >
            <MenuItem value="CLIENT">{t("Client")}</MenuItem>
            <MenuItem value="FOURNISSEUR">{t("Fournisseur")}</MenuItem>
            <MenuItem value="CLIENT_ET_FOURNISSEUR">
              {t("Client et fournisseur")}
            </MenuItem>
          </TextField>
          <TextField
            label={t("Nom / raison sociale")}
            value={form.name}
          ...
          <TextField
            ...
            onChange={(event) => set("taxIdentifier", event.target.value)}
          />
          <TextField
            label={t("Numéro RNE")}
            value={form.rneNumber}
            onChange={(event) => set("rneNumber", event.target.value)}
          />
          <TextField
            label={t("E-mail")}
            type="email"
