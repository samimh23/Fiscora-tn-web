...
function EntryDialog({
  ...
  const { data: costCenters = [] } = useQuery({
    ...
    queryFn: () =>
      api.get<CostCenter[]>(
        `/api/organizations/${organizationId}/dossiers/${dossierId}/cost-centers`,
      ),
  });
  ...
  const mutation = useMutation({
    mutationFn: () =>
      entry
        ? api.put<JournalEntry>(
            `/api/organizations/${organizationId}/dossiers/${dossierId}/entries/${entry.id}`,
            {
              journalId,
              entryDate,
              pieceReference: pieceReference.trim(),
              description: description.trim(),
              lines: lines.map((line) => ({
                accountId: line.accountId,
                label: line.label.trim(),
                debit: line.debit || "0.000",
                credit: line.credit || "0.000",
                thirdPartyName: line.thirdPartyName.trim() || undefined,
    ...
    onError: (reason) =>
      setError(
        reason instanceof ApiError
          ? reason.message
          : `Impossible de ${entry ? "modifier" : "créer"} cette écriture.`,
      ),
  });
  ...
  return (
    ...
    >
      <DialogTitle>
        {entry ? "Corriger l’écriture" : "Nouvelle écriture comptable"}
      </DialogTitle>
      <DialogContent sx={{ pt: "12px !important" }}>
        ...
        <Stack spacing={1.2}>
          {lines.map((line, index) => (
            <Card key={index} variant="outlined" sx={{ p: 1.7 }}>
              <Box
                sx={{
                  display: "grid",
                  gridTemplateColumns: {
                    xs: "1fr",
                    md: "2fr 2fr 1fr 1fr 1.5fr 1.3fr auto",
                  },
                  gap: 1.2,
                  alignItems: "center",
                }}
        ...
        <Card
          ...
          sx={{
            ...
            p: 2,
            bgcolor:
              Math.abs(difference) < 0.0005 && totals.debit > 0
                ? "success.light"
                : "warning.light",
          }}
        >
          ...
          >
            <Typography>
              Total débit <strong>{money(totals.debit)}</strong>
            </Typography>
            <Typography>
              Total crédit <strong>{money(totals.credit)}</strong>
            </Typography>
            <Typography>
              Écart <strong>{money(Math.abs(difference))}</strong>
            </Typography>
          </Stack>
        </Card>
      </DialogContent>
      <DialogActions>
        ...
        >
          {entry ? "Enregistrer les corrections" : "Enregistrer le brouillon"}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
export function EntriesPanel({
  ...
  const filtered = entries.filter(
    (entry) => status === "TOUS" || entry.status === status,
  );
  const refresh = async () =>
    Promise.all([
      ...
      queryClient.invalidateQueries({
        queryKey: ["accounting-report", organizationId, dossierId],
      }),
      queryClient.invalidateQueries({
  ...
  const action = useMutation({
    mutationFn: ({ path, body }: { path: string; body?: unknown }) =>
      api.post<JournalEntry>(
        `/api/organizations/${organizationId}/dossiers/${dossierId}/entries/${path}`,
        body,
      ),
    onSuccess: async () => {
      setError("");
  ...
  return (
    <>
      <Card>
        ...
        >
          <Box>
            <Typography variant="h3">Écritures comptables</Typography>
            <Typography variant="body2" color="text.secondary">
              Saisie → soumission → validation ou rejet → verrouillage
              comptable.
            </Typography>
          </Box>
          <Stack direction="row" spacing={1}>
            ...
            >
              ...
              <MenuItem value="A_VALIDER">À valider</MenuItem>
              <MenuItem value="REJETEE">Rejetées</MenuItem>
              <MenuItem value="COMPTABILISEE">Comptabilisées</MenuItem>
              <MenuItem value="EXTOURNEE">Extournées</MenuItem>
            </TextField>
            {canManage && !archived && (
              <Button
                variant="contained"
        ...
        {!loading && filtered.length === 0 && (
          <Box sx={{ p: 6, textAlign: "center" }}>
            <SwapVertRounded sx={{ fontSize: 46, color: "text.disabled" }} />
          ...
          >
            <Box
              sx={{
                ...
                gridTemplateColumns: {
                  xs: "1fr",
                  lg: "minmax(260px, 1fr) 130px 150px 150px auto",
                },
                gap: 2,
                alignItems: "center",
              }}
            >
              <Box>
                ...
                <Typography variant="caption" color="text.secondary">
                  {shortDate(entry.entryDate)} · {entry.lines.length} ligne(s)
                </Typography>
                {entry.reviewComment && (
              ...
              <Chip
                ...
                size="small"
                color={entryColor(entry.status)}
                variant="outlined"
              />
              <Typography>
                Débit <strong>{money(entry.totalDebit)}</strong>
              </Typography>
              <Typography>
                Crédit <strong>{money(entry.totalCredit)}</strong>
              </Typography>
              <Stack
                direction="row"
                spacing={0.5}
              ...
              >
                {canManage &&
                  !archived &&
                  ["BROUILLON", "REJETEE"].includes(entry.status) && (
                ...
                {canManage &&
                  ...
                  ["BROUILLON", "REJETEE"].includes(entry.status) && (
                    <Button
                      ...
                      onClick={() =>
                        action.mutate({ path: `${entry.id}/submit` })
                      }
                    >
                      Soumettre
                    </Button>
                  )}
                {canPost && !archived && entry.status === "A_VALIDER" && (
                  <>
                    <Button
                      size="small"
                      color="success"
                      ...
                      onClick={() =>
                        action.mutate({ path: `${entry.id}/approve` })
                      }
                    >
                      Valider
                    </Button>
                ...
                {canPost && !archived && entry.status === "COMPTABILISEE" && (
                  <Tooltip title="Extourner aujourd’hui">
                    <IconButton
                      color="warning"
                      onClick={() =>
                        action.mutate({ path: `${entry.id}/reverse` })
                      }
                    >
                      <ReplayRounded />
                    </IconButton>
                ...
                {entry.lines.map((line) => (
                  <Box
                    ...
                    sx={{
                      ...
                      gridTemplateColumns: {
                        xs: "1fr",
                        md: "180px 1fr 130px 130px 100px",
                      },
                      gap: 1.5,
                      borderTop: "1px solid",
                      borderColor: "divider",
                  ...
                  >
                    ...
                    <Typography variant="body2">
                      Débit {money(line.debit)}
                    </Typography>
                    <Typography variant="body2">
                      Crédit {money(line.credit)}
                    </Typography>
                    <Typography variant="body2" color="success.main">
                      {line.letterCode ?? "Non lettré"}
                    </Typography>
                  </Box>
                ))}
              </Box>
      ...
      >
        <DialogTitle>
          Rejeter l’écriture {rejecting?.pieceReference}
        </DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
        ...
        <DialogActions>
          <Button onClick={() => setRejecting(null)}>Annuler</Button>
          <Button
            color="error"
            variant="contained"
