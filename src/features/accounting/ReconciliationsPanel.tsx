...
export function ReconciliationsPanel({
  ...
  const candidateAccounts = useMemo(
    () =>
      accounts.filter((account) =>
        entries.some((entry) =>
          entry.lines.some(
            (line) => line.accountId === account.id && !line.reconciliationId,
          ),
        ),
      ),
    [accounts, entries],
  ...
  const balanced =
    selected.length >= 2 && debit > 0 && Math.abs(debit - credit) < 0.0005;

  const create = useMutation({
    mutationFn: () =>
      api.post<AccountReconciliation>(`${base}/reconciliations`, {
        accountId,
        lineIds: selected,
      }),
    onSuccess: async () => {
    ...
    onError: (reason) =>
      setError(
        reason instanceof ApiError ? reason.message : "Lettrage impossible.",
      ),
  });
  const remove = useMutation({
    ...
    onError: (reason) =>
      setError(
        reason instanceof ApiError ? reason.message : "Délettrage impossible.",
      ),
  });
  ...
  return (
    <Stack spacing={2}>
      ...
      <Card sx={{ p: 2.5 }}>
        ...
        {accountId && (
          <>
                ...
                <TableBody>
                  {candidates.map((line) => (
                    <TableRow key={line.id} hover>
                      ...
                      <TableCell>{shortDate(line.entry.entryDate)}</TableCell>
                      <TableCell>{line.entry.pieceReference}</TableCell>
                      <TableCell>{line.label}</TableCell>
                      <TableCell>{line.thirdPartyName ?? "—"}</TableCell>
            ...
            >
              <Typography>
                Débit <strong>{money(debit)}</strong>
              </Typography>
              <Typography>
                Crédit <strong>{money(credit)}</strong>
              </Typography>
              <Chip
                label={
                  balanced
                    ? "Équilibré"
                    : `Écart ${money(Math.abs(debit - credit))}`
                }
              ...
              <Button
                ...
                disabled={!canPost || archived || !balanced || create.isPending}
      ...
      <Card sx={{ p: 2.5 }}>
        ...
        {(reconciliations.data ?? []).map((item) => (
          <Box
            ...
            sx={{
              ...
              gridTemplateColumns: {
                xs: "1fr",
                md: "150px 1fr 150px 130px auto",
              },
              gap: 2,
              alignItems: "center",
            }}
          >
            <Chip label={item.code} color="success" variant="outlined" />
            <Typography>
              {item.account.code} — {item.account.name}
            </Typography>
            <Typography>{shortDate(item.reconciliationDate)}</Typography>
            <Typography>{money(item.totalDebit)}</Typography>
            <Button
              color="error"
              startIcon={<LinkOffRounded />}
              disabled={!canPost || archived || remove.isPending}
        ...
        {!reconciliations.isLoading && !reconciliations.data?.length && (
