...
export function ClientPortalShell() {
  ...
  const drawer = (
    ...
    >
      ...
      <Box sx={{ px: 3, pb: 2 }}>
        <Typography
          variant="overline"
          sx={{ color: "#f2c56b", letterSpacing: ".16em" }}
        >
          Espace client
        </Typography>
        <Typography
          variant="body2"
          sx={{ color: "rgba(255,255,255,.66)", mt: 0.5 }}
        >
          Vos documents et échéances, au même endroit.
        </Typography>
      </Box>
          ...
          return (
            <ListItemButton
              ...
              sx={{
                ...
                "&.Mui-selected": { bgcolor: "#f2c56b", color: "#153a30" },
      ...
      <Box sx={{ p: 2.5, borderTop: "1px solid rgba(255,255,255,.1)" }}>
        <Box sx={{ display: "flex", gap: 1.2, alignItems: "center" }}>
          <SupportAgentOutlined sx={{ color: "#f2c56b" }} />
          <Box>
  ...
  return (
    <Box sx={{ display: "flex", minHeight: "100vh", bgcolor: "#f5f3ec" }}>
      <AppBar
        position="fixed"
        elevation={0}
        sx={{
          bgcolor: "rgba(255,253,248,.96)",
          color: "text.primary",
          borderBottom: "1px solid",
          borderColor: "divider",
          ml: desktop && direction === "ltr" ? `${width}px` : 0,
          mr: desktop && direction === "rtl" ? `${width}px` : 0,
      ...
      >
        <Toolbar sx={{ minHeight: "72px !important", gap: 2 }}>
          ...
          <Tooltip title={t("Notifications")}>
            <IconButton component={NavLink} to="/portail/notifications">
              <NotificationsNoneOutlined />
            </IconButton>
          </Tooltip>
          <IconButton onClick={(e) => setAccount(e.currentTarget)}>
            <Avatar sx={{ bgcolor: "#17614e", width: 38, height: 38 }}>
          ...
          >
            ...
            <MenuItem
              component={NavLink}
              to="/portail/parametres"
              onClick={() => setAccount(null)}
            >
              <ManageAccountsOutlined sx={{ mr: 1 }} />
              {t("Mon compte")}
            </MenuItem>
      ...
      <Box component="nav" sx={{ width: { lg: width }, flexShrink: { lg: 0 } }}>
        <Drawer
          ...
          sx={{ "& .MuiDrawer-paper": { width, border: 0 } }}
        >
          {drawer}
        </Drawer>
      </Box>
      <Box component="main" sx={{ flex: 1, minWidth: 0, pt: "72px" }}>
        <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1480, mx: "auto" }}>
          <Outlet />
        </Box>
      </Box>
    </Box>
