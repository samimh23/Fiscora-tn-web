...
export function ClientPortalDossiersPage() {
  ...
  const visible = useMemo(() => {
    ...
    return (dossiers.data?.items ?? []).filter(
      (item) =>
        !value ||
        [
          item.legalName,
  ...
  return (
    <>
      ...
      <Typography color="text.secondary" sx={{ mb: 3 }}>
        Vous ne voyez que les entreprises auxquelles votre cabinet vous a
        explicitement donné accès.
      </Typography>
      <TextField
        fullWidth
        value={search}
      ...
      />
      {dossiers.isError && (
        <Alert severity="error">Impossible de charger vos dossiers.</Alert>
      )}
        ...
        <Card>
          <CardContent sx={{ p: 6, textAlign: "center" }}>
            ...
            <Typography color="text.secondary">
              Votre cabinet peut vous attribuer une société depuis son espace
              équipe.
            </Typography>
          </CardContent>
        </Card>
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              >
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  >
                    <Typography variant="h4" sx={{ fontSize: 23 }}>
                      {dossier.legalName}
                    </Typography>
                    <Chip
                      size="small"
                      label={
                        dossier.status === "ACTIF" ? "Actif" : dossier.status
                      }
                      color="success"
                      variant="outlined"
                    />
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {dossier.tradeName ||
                      ...
                      "Entreprise"}
                    {dossier.taxIdentifier
                      ? ` · MF ${dossier.taxIdentifier}`
                      : ""}
                  </Typography>
                </Box>
              </Stack>
              <Button
                ...
                to={`/portail/dossiers/${dossier.id}`}
                variant="contained"
