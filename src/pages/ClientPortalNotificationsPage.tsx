...
export function ClientPortalNotificationsPage() {
  ...
  const query = useQuery({
    queryKey: ["client-notifications", id],
    queryFn: () =>
      api.get<NotificationItem[]>(`/api/organizations/${id}/notifications`),
    enabled: Boolean(id),
  });
  const mark = useMutation({
    mutationFn: (notificationId: string) =>
      api.patch(
        `/api/organizations/${id}/notifications/${notificationId}/read`,
      ),
    onSuccess: () =>
  ...
  const all = useMutation({
    mutationFn: () =>
      api.patch(`/api/organizations/${id}/notifications/read-all`),
    onSuccess: () =>
      void qc.invalidateQueries({ queryKey: ["client-notifications"] }),
  });
  return (
    <>
      <PageHeader
        eyebrow="Suivi"
        title="Notifications"
        description="Messages du cabinet, échéances et mises à jour importantes."
        action={
          <Button
            startIcon={<DoneAllRounded />}
            onClick={() => all.mutate()}
            disabled={!query.data?.some((x) => !x.readAtUtc)}
          >
              ...
              >
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  >
                    ...
                    {!item.readAtUtc && (
                      <Chip label="Nouveau" size="small" color="secondary" />
                    )}
                  </Stack>
                ...
                {!item.readAtUtc && (
                  <Button size="small" onClick={() => mark.mutate(item.id)}>
                    Marquer comme lu
                  </Button>
                )}
              </Box>
