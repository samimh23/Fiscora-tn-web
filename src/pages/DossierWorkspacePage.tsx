...
> = {
  tasks: {
    ...
    title: "Tâches",
    description:
      "Planifiez, affectez et validez le travail du dossier sélectionné.",
  },
  obligations: {
    eyebrow: "Calendrier fiscal",
    title: "Obligations",
    description:
      "Préparez, validez, déposez et suivez les échéances du client.",
  },
  documents: {
  ...
  banking: {
    ...
    title: "Banque",
    description:
      "Importez les relevés et rapprochez les mouvements avec la comptabilité.",
  },
  declarations: {
    eyebrow: "Fiscalité périodique",
    title: "Déclarations",
    description:
      "Préparez la déclaration mensuelle depuis les factures comptabilisées.",
  },
};
          ...
          <Box>
            <Stack direction="row" spacing={1} sx={{ mb: 1, flexWrap: "wrap" }}>
              <Chip
                ...
                color={dossier.status === "ACTIVE" ? "success" : "default"}
                size="small"
              />
              <Chip
                label={dossier.legalForm || "Forme non renseignée"}
                size="small"
                variant="outlined"
              />
              {dossier.isVatSubject && (
                <Chip label="TVA" size="small" variant="outlined" />
              )}
              {dossier.hasVatSuspension && (
                <Chip label="Suspension TVA" size="small" variant="outlined" />
              )}
              {dossier.isTotallyExporting && (
                <Chip label="Export total" size="small" variant="outlined" />
              )}
            </Stack>
            <Typography
              variant="h3"
              sx={{ fontSize: { xs: 24, md: 30 }, lineHeight: 1.1 }}
            >
              {dossier.legalName}
            </Typography>
            <Typography color="text.secondary" sx={{ mt: 0.75 }}>
              {[
                dossier.tradeName,
                dossier.taxIdentifier && `MF ${dossier.taxIdentifier}`,
    ...
    <Card>
      <CardContent sx={{ p: { xs: 3, md: 5 }, textAlign: "center" }}>
        ...
        <Typography variant="h3">
          Choisissez un dossier pour démarrer
        </Typography>
        <Typography
          color="text.secondary"
          sx={{ mt: 1, maxWidth: 620, mx: "auto" }}
        >
          Toutes les actions de production sont liées à un client précis.
          Sélectionnez un dossier en haut de l’écran pour afficher son contexte,
          ses tâches et les prochaines étapes.
        </Typography>
      </CardContent>
    </Card>
  );
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <DossierSelector value={dossierId} onChange={handleDossierChange} />
        }
      />
      ...
      {dossier.data && (
        <DossierContextCard dossier={dossier.data} module={module} />
      )}
      {dossier.data && module === "tasks" && (
        <DossierTasksPanel
      ...
      {dossier.data && module === "obligations" && (
