...
export function AnnualTaxPage() {
  ...
  const finalize = useMutation({
    ...
    onError: (reason) =>
      setError(reason instanceof Error ? reason.message : "Clôture impossible."),
  });
  ...
  const exportAnnex = async (kind: "amortissements" | "retenues-source", format: "pdf" | "csv") => {
    if (!base) return;
    await downloadApiFile(
  ...
  return (
    <>
        ...
        <CardContent>
          ...
          <Stack direction={{ xs: "column", md: "row" }} spacing={2} sx={{ mt: 2 }}>
            <TextField
              ...
              onChange={(event) => setForm({ ...form, acomptesVerses: event.target.value })}
            />
            <TextField
              label="Excédents antérieurs"
              helperText="Crédit reporté d'un exercice précédent"
              value={form.excedentsAnterieurs}
              onChange={(event) => setForm({ ...form, excedentsAnterieurs: event.target.value })}
            />
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
          ...
          <Stack direction="row" spacing={1.5} sx={{ mt: 3, flexWrap: "wrap" }}>
            ...
            <Button startIcon={<DownloadRounded />} disabled={!base} onClick={() => void exportFile("pdf")}>Export pré-liasse PDF</Button>
            <Button startIcon={<DownloadRounded />} disabled={!base} onClick={() => void exportFile("csv")}>Export pré-liasse CSV</Button>
          </Stack>
          <Stack direction="row" spacing={1.5} sx={{ mt: 1.5, flexWrap: "wrap" }}>
            <Button size="small" startIcon={<DownloadRounded />} disabled={!base} onClick={() => void exportAnnex("amortissements", "pdf")}>Annexe amortissements PDF</Button>
            <Button size="small" startIcon={<DownloadRounded />} disabled={!base} onClick={() => void exportAnnex("amortissements", "csv")}>CSV</Button>
            <Button size="small" startIcon={<DownloadRounded />} disabled={!base} onClick={() => void exportAnnex("retenues-source", "pdf")}>Annexe retenues à la source PDF</Button>
            <Button size="small" startIcon={<DownloadRounded />} disabled={!base} onClick={() => void exportAnnex("retenues-source", "csv")}>CSV</Button>
          </Stack>
        </CardContent>
      </Card>
        ...
        <Card sx={{ mt: 2 }}>
          <CardContent>
            <Stack direction="row" spacing={1} sx={{ alignItems: "center", mb: 2 }}>
              <Alert severity="warning" sx={{ flex: 1 }}>{report.warning}</Alert>
              {report.finalized && <Chip color="success" label="Exercice clôturé" />}
            </Stack>
            ...
            <Typography variant="h6" sx={{ mb: 1 }}>Tableau de détermination du résultat fiscal</Typography>
            <Table size="small">
              <TableBody>
                {[
            ...
            <Typography variant="h6" sx={{ mt: 3, mb: 1 }}>Liquidation de l'impôt dû</Typography>
            <Table size="small">
              <TableBody>
                {[
                  ["Taux IS retenu", report.fiscal.corporateTaxRate],
                  ["IS brut", report.fiscal.grossCorporateTax],
                  [report.fiscal.regime === "FORFAITAIRE" ? "Forfaitaire" : "Minimum d'impôt", report.fiscal.regime === "FORFAITAIRE" ? report.fiscal.forfaitaireTax : report.fiscal.minimumTax],
            ...
            <Typography variant="h6" sx={{ mt: 3, mb: 1 }}>Régularisation</Typography>
            <Table size="small">
              <TableBody>
                <TableRow>
                  <TableCell>Retenue à la source subie (auto, voir annexe)</TableCell>
                  <TableCell align="right"><Money value={report.regularisation.retenueALaSourceSubie} /></TableCell>
                </TableRow>
                <TableRow>
                  <TableCell>Acomptes provisionnels versés</TableCell>
                  <TableCell align="right"><Money value={report.regularisation.acomptesVerses} /></TableCell>
                </TableRow>
                <TableRow>
                  <TableCell>Excédents antérieurs</TableCell>
                  <TableCell align="right"><Money value={report.regularisation.excedentsAnterieurs} /></TableCell>
                </TableRow>
                <TableRow>
                  <TableCell>Autres crédits imputables</TableCell>
                  <TableCell align="right"><Money value={report.regularisation.autresCredits} /></TableCell>
                </TableRow>
                <TableRow>
                  <TableCell sx={{ fontWeight: 700 }}>
                    {report.regularisation.sens === "REPORT" ? "Report (crédit à reporter)" : "Net à payer (IV)"}
                  </TableCell>
            ...
            <Typography variant="h6" sx={{ mt: 3 }}>Acomptes provisionnels (exercice suivant)</Typography>
            <Table size="small">
              <TableHead>
                <TableRow>
      ...
      <Card sx={{ mt: 2 }}>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 1 }}>Déficits reportés</Typography>
          <Typography color="text.secondary" sx={{ mb: 2 }}>
            Pertes fiscales des exercices clôturés, imputables sur les 4 exercices suivants.
          </Typography>
          <QueryState
              ...
              <TableBody>
                {deficits.data?.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell align="right"><Money value={row.originalAmount} /></TableCell>
                    <TableCell align="right"><Money value={row.remainingAmount} /></TableCell>
                    <TableCell>{row.expiresAfterYear}</TableCell>
                    <TableCell>
                      <Chip
                        size="small"
                        label={row.status === "DISPONIBLE" ? "Disponible" : "Épuisé"}
                        color={row.status === "DISPONIBLE" ? "success" : "default"}
                      />
                    </TableCell>
                  </TableRow>
                ))}
