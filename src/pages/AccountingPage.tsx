...
export function AccountingPage() {
  ...
  const accounts = useQuery({
    ...
    queryFn: () =>
      api.get<LedgerAccount[]>(`${base}/ledger-accounts?includeInactive=true`),
    enabled: Boolean(
      organizationId && dossierId && can("chart_of_accounts.view"),
    ),
  });
  ...
  const costCenters = useQuery({
    ...
    queryFn: () =>
      api.get<CostCenter[]>(`${base}/cost-centers?includeInactive=true`),
    enabled: Boolean(
      organizationId && dossierId && can("chart_of_accounts.view"),
    ),
  });
  const costCenterReport = useQuery({
    ...
    queryFn: () =>
      api.get<CostCenterReportRow[]>(
        `${base}/cost-centers/report?from=${reportFrom}&to=${reportTo}`,
      ),
    enabled: Boolean(
      organizationId &&
      dossierId &&
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return editing
        ? api.put(`${base}/ledger-accounts/${editing.id}`, payload)
        : api.post(`${base}/ledger-accounts`, payload);
    },
    onSuccess: async () => {
      await refresh();
      setDialogOpen(false);
  ...
  const install = useMutation({
    mutationFn: () =>
      api.post<{
        ...
        total: number;
        reference: string;
      }>(`${base}/ledger-accounts/apply-tunisian-chart`),
    onSuccess: refresh,
  });
  const saveCc = useMutation({
    mutationFn: () => {
      ...
      return editingCc
        ? api.put(`${base}/cost-centers/${editingCc.id}`, payload)
        : api.post(`${base}/cost-centers`, payload);
    },
    onSuccess: async () => {
  ...
  const filtered = useMemo(
    () =>
      (accounts.data ?? []).filter((item) => {
        const matches = `${item.code} ${item.name}`
          .toLowerCase()
    ...
    <>
      ...
      {!dossierId && (
        <Alert severity="info" sx={{ mb: 2 }}>
          Choisissez d’abord un dossier : son plan comptable et ses exercices
          sont totalement séparés des autres clients.
        </Alert>
      )}
      {error && (
      ...
      {install.data && (
        <Alert severity="success" sx={{ mb: 2 }}>
          Plan {install.data.reference} installé : {install.data.added}{" "}
          compte(s) ajouté(s), {install.data.skipped} déjà présent(s).
        </Alert>
      )}
      <Card sx={{ mb: 2 }}>
          ...
          <CardContent sx={{ p: 0 }}>
            ...
            >
              ...
              <FormControlLabel
                ...
                label="Afficher les comptes inactifs"
              />
              <Box sx={{ flex: 1 }} />
              <Button
                startIcon={<AutoAwesomeRounded />}
                disabled={!can("chart_of_accounts.manage") || install.isPending}
                onClick={() => install.mutate()}
              >
                Installer le plan tunisien NC 01
              </Button>
              <Button
                variant="contained"
                startIcon={<AddRounded />}
                disabled={!can("chart_of_accounts.manage")}
                onClick={openCreate}
              >
                Nouveau compte
              </Button>
            ...
            <Alert severity="info" sx={{ mx: 2.5, mb: 2 }}>
              Le référentiel général NC 01 est installé sans écraser vos
              subdivisions personnalisées. Le responsable comptable peut ajouter
              des sous-comptes adaptés à l’activité du client.
            </Alert>
            <QueryState
              loading={accounts.isLoading}
              error={accounts.isError}
              empty={!filtered.length}
              emptyText="Le plan du dossier est vide. Installez le plan tunisien ou créez un compte."
            />
            {filtered.length > 0 && (
                  ...
                  <TableBody>
                    {filtered.map((account) => (
                      <TableRow key={account.id}>
                        ...
                        <TableCell>
                          {account.allowsPosting ? "Autorisée" : "Collectif"}
                        </TableCell>
                        <TableCell>
                          <Chip
                            size="small"
        ...
        <>
          <Card sx={{ mb: 2 }}>
            <CardContent sx={{ p: 0 }}>
              ...
              >
                <Typography sx={{ fontWeight: 700 }}>
                  Centres de coût du dossier
                </Typography>
                <Button
                  variant="contained"
                  startIcon={<AddRounded />}
                  disabled={!can("chart_of_accounts.manage")}
                  onClick={openCreateCc}
                >
                  Nouveau centre de coût
                </Button>
              </Box>
              <Alert severity="info" sx={{ mx: 2.5, mb: 2 }}>
                Taguez chaque ligne d’écriture (module Écritures comptables)
                avec un centre de coût pour obtenir un compte de résultat par
                département, projet ou site.
              </Alert>
              <QueryState
                loading={costCenters.isLoading}
                error={costCenters.isError}
                empty={!costCenters.data?.length}
                emptyText="Aucun centre de coût. Créez-en un pour commencer à ventiler vos écritures."
              />
              {Boolean(costCenters.data?.length) && (
                <Box sx={{ overflowX: "auto" }}>
                  <Table size="small">
                    <TableHead>
                      <TableRow>
          ...
          <Card>
            <CardContent sx={{ p: 0 }}>
              ...
              <QueryState
                loading={costCenterReport.isLoading}
                error={costCenterReport.isError}
                empty={!costCenterReport.data?.length}
                emptyText="Aucune écriture comptabilisée sur cette période."
              />
              {Boolean(costCenterReport.data?.length) && (
                <Box sx={{ overflowX: "auto" }}>
                  <Table size="small">
                    <TableHead>
                      <TableRow>
                    ...
                    <TableBody>
                      {costCenterReport.data?.map((row, i) => (
                        <TableRow key={i}>
                          <TableCell>
                            {row.code
                              ? `${row.code} — ${row.name}`
      ...
      {tab === "costcenters" && !dossierId && (
        <Alert severity="info">Choisissez un dossier client.</Alert>
      )}
      ...
      <Dialog
        open={dialogOpen}
        onClose={save.isPending ? undefined : () => setDialogOpen(false)}
        fullWidth
        maxWidth="sm"
      >
        <DialogTitle>
          {editing ? "Modifier le compte" : "Nouveau compte comptable"}
        </DialogTitle>
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
              ...
              <TextField
                ...
                onChange={(event) =>
                  setForm({ ...form, normalBalance: event.target.value })
                }
              >
            ...
            <TextField
              ...
              onChange={(event) =>
                setForm({ ...form, parentAccountId: event.target.value })
              }
            >
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={(event) =>
                    setForm({ ...form, allowsPosting: event.target.checked })
                  }
                />
              }
              label="Autoriser la saisie directe sur ce compte"
            />
            {editing && (
              <FormControlLabel
                control={
      ...
      <Dialog
        open={ccDialogOpen}
        onClose={saveCc.isPending ? undefined : () => setCcDialogOpen(false)}
        fullWidth
        maxWidth="sm"
      >
        <DialogTitle>
          {editingCc ? "Modifier le centre de coût" : "Nouveau centre de coût"}
        </DialogTitle>
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
              <TextField
                ...
                value={ccForm.code}
                onChange={(event) =>
                  setCcForm({ ...ccForm, code: event.target.value })
                }
              />
              <TextField
                fullWidth
                ...
                value={ccForm.name}
                onChange={(event) =>
                  setCcForm({ ...ccForm, name: event.target.value })
                }
              />
            </Stack>
            <TextField
              ...
              onChange={(event) =>
                setCcForm({ ...ccForm, description: event.target.value })
              }
            />
            {editingCc && (
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    onChange={(event) =>
                      setCcForm({ ...ccForm, isActive: event.target.checked })
                    }
                  />
                }
                label="Centre de coût actif"
              />
            )}
          </Stack>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setCcDialogOpen(false)}>Annuler</Button>
          <Button
            variant="contained"
            disabled={
              !ccForm.code.trim() || !ccForm.name.trim() || saveCc.isPending
            }
            onClick={() => saveCc.mutate()}
          >
            Enregistrer
          </Button>
        </DialogActions>
