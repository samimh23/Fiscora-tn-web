...
type ActionKind =
  "documents" | "deadlines" | "payments" | "messages" | "approvals";
...
export function ClientPortalDashboardPage() {
  ...
  const workspace = useQuery({
    queryKey: [
      "client-portal-action-center",
      organizationId,
      dossiers.data?.items.map((x) => x.id).join(","),
    ],
    enabled: Boolean(organizationId && dossiers.data),
    queryFn: async (): Promise<DossierWorkspace[]> =>
      Promise.all(
        (dossiers.data?.items ?? []).map(async (dossier) => {
          ...
          const [expectations, obligations, fees, declarations, approvals] =
            await Promise.all([
              safe(
              ...
              safe(
                api.get<ClientApproval[]>(`${base}/client-portal/approvals`),
                [],
              ),
            ]);
          return {
  ...
  const actions = useMemo(() => {
    ...
    for (const item of workspace.data ?? []) {
      ...
      for (const expectation of item.expectations.filter(
        (x) =>
          !["RECUE", "VALIDEE", "ANNULEE"].includes(x.status ?? "") &&
          !x.receivedDocumentId,
      )) {
        result.push({
          id: `doc-${expectation.id}`,
          dossierName: item.dossier.legalName,
          kind: "documents",
          urgent: expectation.status === "REJETEE",
      ...
      for (const obligation of item.obligations.filter(
        (x) =>
          !["DEPOSE", "PAYEE", "VALIDEE"].includes(x.status) &&
          (x.isLate ||
            new Date(`${x.dueOn}T00:00:00`).getTime() <=
              today.getTime() + 30 * 86_400_000),
      )) {
        result.push({
          id: `due-${obligation.id}`,
          dossierName: item.dossier.legalName,
          kind: "deadlines",
          urgent: obligation.isLate,
      ...
      for (const invoice of item.fees.filter(
        (x) =>
    ...
    for (const notification of notifications.data ?? []) {
      if (notification.type === "MESSAGE_PORTAIL_CLIENT") {
        result.push({
          ...
          dossierName: organization?.name ?? "Votre cabinet",
          kind: "messages",
          urgent: false,
          title: notification.title,
          detail: notification.message,
          link: "/portail/notifications",
        });
      }
    }
    ...
    <>
      ...
      >
        <Box>
          <Typography
            variant="overline"
            color="primary"
            sx={{ fontWeight: 700, letterSpacing: ".15em" }}
          >
          ...
          <Typography color="text.secondary">
            Voici ce qui demande votre attention chez {organization?.name}.
          </Typography>
        </Box>
        <Stack
      ...
      <Card>
        <Box sx={{ px: { xs: 2, md: 3 }, pt: 2.5 }}>
          ...
          >
            <Box>
              <Typography variant="h3">Mes actions</Typography>
              <Typography variant="body2" color="text.secondary">
                Un seul endroit pour savoir quoi transmettre, valider ou régler.
              </Typography>
            </Box>
            <Chip
              label={`${visibleActions.length} action${visibleActions.length > 1 ? "s" : ""}`}
              color={
                visibleActions.some((x) => x.urgent) ? "warning" : "default"
              }
            />
          </Stack>
          <Tabs
        ...
        {(dossiers.isLoading || workspace.isLoading) && (
          <Box sx={{ p: 4 }}>
            <Typography color="text.secondary">
              Préparation de votre liste d’actions…
            </Typography>
          </Box>
        )}
        {!workspace.isLoading && !visibleActions.length && (
          <Box sx={{ p: 6, textAlign: "center" }}>
            <DoneAllRounded color="success" sx={{ fontSize: 50 }} />
            <Typography variant="h4" sx={{ mt: 1.5 }}>
              Tout est à jour
            </Typography>
            <Typography color="text.secondary">
              Aucune action ne vous est demandée pour le moment.
            </Typography>
          </Box>
        )}
            ...
            >
              <Box
                sx={{
                  ...
                  height: 46,
                  borderRadius: 2.5,
                  bgcolor: action.urgent ? "#fff0e7" : "#e8f1ed",
                  color: action.urgent ? "warning.main" : "primary.main",
                  display: "grid",
                  placeItems: "center",
                  flexShrink: 0,
                }}
              ...
              <Box sx={{ flex: 1 }}>
                ...
                >
                  ...
                  {action.urgent && (
                    <Chip size="small" color="warning" label="Prioritaire" />
                  )}
                </Stack>
                <Typography variant="body2" color="text.secondary">
                  {action.dossierName} · {action.detail}
                </Typography>
              </Box>
              <Button
                component={RouterLink}
      ...
      >
        ...
        <DialogContent>
          <Stack spacing={2.5} sx={{ pt: 1 }}>
            <Alert severity="info">
              Vous pouvez sélectionner plusieurs fichiers. Chaque pièce restera
              rattachée à la société choisie.
            </Alert>
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(event) => setUploadDossierId(event.target.value)}
              >
            ...
            <Stack direction="row" spacing={2}>
              <TextField fullWidth label="Année" value={year} disabled />
              <TextField
                fullWidth
                label="Mois"
            ...
            {upload.isError && (
              <Alert severity="error">
                Le dépôt n’a pas pu être terminé. Réessayez ou contactez votre
                cabinet.
              </Alert>
            )}
          </Stack>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            disabled={!uploadDossierId || !files.length || upload.isPending}
