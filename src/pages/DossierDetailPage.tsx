...
function InfoItem({
  ...
  return (
    <Box>
      ...
      <Typography sx={{ fontWeight: 700, mt: 0.2 }}>{value || "—"}</Typography>
    </Box>
  );
}
...
export function DossierDetailPage() {
  ...
  const [tab, setTab] = useState(
    searchParams.get("espace") === "suivi" ? "tasks" : "overview",
  );
  const [editOpen, setEditOpen] = useState(false);
  const [contactOpen, setContactOpen] = useState(false);
  const [selectedContact, setSelectedContact] = useState<DossierContact | null>(
    null,
  );
  const [assignmentOpen, setAssignmentOpen] = useState(false);
  const [selectedAssignment, setSelectedAssignment] =
    useState<DossierAssignment | null>(null);
  ...
  const members = useQuery({
    ...
    enabled: Boolean(
      organizationId && can("dossiers.assign") && can("users.view"),
    ),
  });

  const archive = useMutation({
    mutationFn: () =>
      api.post<DossierSummary>(
        `/api/organizations/${organizationId}/dossiers/${dossierId}/archive`,
      ),
    onSuccess: async () => {
  ...
  if (!can("dossiers.view"))
    return (
      <Alert severity="error">
        Vous n’avez pas l’autorisation de consulter les dossiers clients.
      </Alert>
    );
  ...
  const dossierSections = [
    {
      ...
      items: [
        { value: "overview", label: "Synthèse" },
        {
          value: "contacts",
          label: `Contacts (${contacts.data?.length ?? 0})`,
        },
        ...(can("dossiers.assign")
          ? [
              {
                value: "team",
                label: `Équipe (${assignments.data?.filter((entry) => entry.isActive).length ?? 0})`,
              },
            ]
          : []),
      ],
    },
    {
      ...
      items: [
        ...(can("documents.view")
          ...
          : []),
        ...(can("client_portal.view")
          ? [{ value: "client-messages", label: "Échanges client" }]
          : []),
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        ...(can("obligations.view")
          ? [{ value: "obligations", label: "Obligations fiscales" }]
          : []),
      ],
    },
    {
      ...
      items: [
        ...(can("tasks.view")
          ? [{ value: "tasks", label: "Tâches et validations" }]
          : []),
      ],
    },
  ].filter((section) => section.items.length);
  const activeSection =
    dossierSections.find((section) =>
      section.items.some((entry) => entry.value === tab),
    ) ?? dossierSections[0];
    ...
    <>
      ...
      <PageHeader
        ...
        description={
          [
            item.tradeName,
            item.taxIdentifier && `MF ${item.taxIdentifier}`,
            item.activitySector,
          ]
            .filter(Boolean)
            .join(" · ") || "Informations générales du client"
        }
        action={
      ...
      <Card sx={{ mb: 2 }}>
        <Tabs
          ...
          onChange={(_, value: string) => {
            const section = dossierSections.find(
              (entry) => entry.key === value,
            );
            if (section?.items[0]) setTab(section.items[0].value);
          }}
          variant="scrollable"
          scrollButtons="auto"
          ...
          >
            {activeSection.items.map((entry) => (
              <Button
                key={entry.value}
                size="small"
                variant={tab === entry.value ? "contained" : "text"}
        ...
        <Card>
          <CardContent sx={{ p: 0 }}>
            ...
            >
              ...
              {can("dossiers.contacts.manage") && !archived && (
                <Button
                  variant="contained"
            ...
            {!contacts.isLoading && !contacts.data?.length && (
              <Box sx={{ p: 6, textAlign: "center" }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Ajoutez le gérant ou votre interlocuteur principal.
                </Typography>
              </Box>
            )}
            {contacts.data?.map((contact) => (
              <Box
                ...
                sx={{
                  ...
                  borderTop: "1px solid",
                  borderColor: "divider",
                  opacity: contact.isActive ? 1 : 0.55,
                  display: "flex",
                  gap: 2,
                  alignItems: "center",
                }}
              >
                <Box
                  sx={{
                    ...
                    bgcolor: contact.isPrimary
                      ? "secondary.main"
                      : "primary.light",
                    color: contact.isPrimary ? "#fff" : "primary.main",
                    borderRadius: 3,
                    display: "grid",
                    placeItems: "center",
                    fontWeight: 800,
                ...
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    ...
                    {contact.isPrimary && (
                      <Chip label="Principal" size="small" color="secondary" />
                    )}
                    {!contact.isActive && <Chip label="Inactif" size="small" />}
                  </Box>
                  <Typography variant="body2" color="text.secondary">
                    {contact.role || "Contact client"}
                  </Typography>
          ...
          <CardContent sx={{ p: 0 }}>
            ...
            {!assignments.isLoading && !assignments.data?.length && (
              <Box sx={{ p: 6, textAlign: "center" }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Affectez un responsable et, si nécessaire, des personnes en
                  support.
                </Typography>
              </Box>
            )}
            {assignments.data?.map((assignment) => (
              ...
              >
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {assignment.email} · {assignment.cabinetRole}
                  </Typography>
                </Box>
                <Box
                  sx={{
      ...
      <Dialog
        open={archiveOpen}
        onClose={archive.isPending ? undefined : () => setArchiveOpen(false)}
        maxWidth="xs"
        fullWidth
      >
        ...
        <DialogContent>
          <Typography>
            Le dossier <strong>{item.legalName}</strong> deviendra en lecture
            seule. Son historique restera conservé.
          </Typography>
          {archive.isError && (
